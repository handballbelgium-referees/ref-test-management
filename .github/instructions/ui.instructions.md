...
- In services, prefer `@Service({ providedIn: 'root' })` in Angular 22+ for true singletons (or keep equivalent `@Injectable` metadata where needed), prefer `inject()`, and avoid unmanaged subscriptions. Prefer the async pipe for template streams.
